...
from transformers import AutoModel, AutoTokenizer, AutoConfig, T5Config, MT5Config, LongT5Config
import json
...
class Transformer(nn.Module):
    ...
    def _load_model(self, model_name_or_path, config, cache_dir, **model_args):
        ...
        if isinstance(config, T5Config):
            self._load_t5_model(model_name_or_path, config,
                                cache_dir, **model_args)
        elif isinstance(config, MT5Config):
            self._load_mt5_model(model_name_or_path, config,
                                 cache_dir, **model_args)
        elif isinstance(config, LongT5Config):
            self._load_longt5_model(
    ...
    def _load_longt5_model(self, model_name_or_path, config, cache_dir, **model_args):
        """Loads the encoder model from T5"""
